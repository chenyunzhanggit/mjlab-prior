"""
MuJoCo sim2sim deployment for G1 kicking VQ task.

Usage:
  cd /home/dx/motionprior
  python deploy_mujoco/deploy_mujoco_kicking.py \\
      --config deploy_mujoco/configs/g1_kicking_vq.yaml

  python deploy_mujoco/deploy_mujoco_kicking.py \\
      --policy_path logs/g1_kicking_vq/.../model_XXXX.pt
...
Episode 流程:
  1. 机器人随机朝向 θ
  2. 球置于机器人正前方 0.8~1.8m、侧向偏移 ±(0.3~0.6)m，静止
  3. 目标（球门）置于机器人正前方 6~10m
  4. 机器人接近并将球踢向目标
  5. 按 R 手动重置
"""
...
from kicking_controller import KickingVQController
...
# ---------------------------------------------------------------------------
# MujocoSimulator subclass with ball support (kicking physics)
# ---------------------------------------------------------------------------

class MujocoSimulatorWithBall(MujocoSimulator):
    """Extends MujocoSimulator with soccer ball freejoint support.

    Kicking physics: linear_damping=0.4, angular_damping=0.2
    """

    _BALL_LINEAR_DAMPING  = 0.4
    ...
    def step(self, target_q, kps, kds):
        ...
        # PhysX-matched damping: linear_damping=0.4, angular_damping=0.2
...
def build_config(args) -> dict:
    ...
    cfg.setdefault("xml_path", os.path.join(_HERE, "xml", "g1_29dof_ball_kicking.xml"))
    ...
    # Ball/goal init ranges — must match G1KickingEnv._reset_scene_along_line
    cfg.setdefault("ball_fwd_range",  [0.8, 1.8])   # ball forward distance from robot [m]
    cfg.setdefault("ball_lat_range",  [0.3, 0.6])   # ball lateral offset magnitude [m] (random sign)
    cfg.setdefault("goal_dist_range", [6.0, 10.0])  # goal forward distance from robot [m]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MuJoCo sim2sim for G1 kicking VQ policy",
...
def main():
    ...
    print("=" * 60)
    print("  G1 Kicking VQ – MuJoCo sim2sim")
    print("=" * 60)
    ...
    print(f"  ball_fwd  : {cfg['ball_fwd_range']} m (robot forward)")
    print(f"  ball_lat  : ±{cfg['ball_lat_range']} m (lateral offset)")
    print(f"  goal_dist : {cfg['goal_dist_range']} m (robot forward)")
    print("=" * 60)

    rng = np.random.default_rng()

    # ---- Controller ----
    controller = KickingVQController(
    ...
    _fwd_range  = tuple(cfg["ball_fwd_range"])
    _lat_range  = tuple(cfg["ball_lat_range"])
    _goal_range = tuple(cfg["goal_dist_range"])
    ...
    def do_reset():
        ...
        # Random robot facing direction — matches training (G1KickingEnv samples random theta)
        ...
        # Ball: d_fwd ∈ [0.8, 1.8]m forward, d_lat ∈ [0.3, 0.6]m lateral (random sign)
        d_fwd = rng.uniform(*_fwd_range)
        d_lat = rng.uniform(*_lat_range) * (1.0 if rng.random() > 0.5 else -1.0)
        ball_pos = np.array([
            robot_pos[0] + d_fwd * cos_t - d_lat * sin_t,
            robot_pos[1] + d_fwd * sin_t + d_lat * cos_t,
            SOCCER_BALL_RADIUS,
        ], dtype=np.float32)
        simulator.reset_ball(ball_pos)

        # Goal: 6~10m forward along theta
        ...
        print(f"[Reset] theta={np.degrees(theta):.1f}°  "
              ...
              f"d_fwd={d_fwd:.2f}m  d_lat={d_lat:+.2f}m  "
