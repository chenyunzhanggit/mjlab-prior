"""
Kicking VQ Policy controller for MuJoCo sim2sim deployment.

Observation layout (from G1KickingObservationsCfg.PolicyObsCfg, 383-dim):
...
class _KickingVQPolicyInference(nn.Module):
    """Deployment-only reconstruction of DownStreamVQPolicy for kicking task."""
...
class KickingVQController(BaseController):
    """
    Kicking VQ policy controller for MuJoCo sim2sim.
    ...
    def __init__(
        ...
        self._policy = _KickingVQPolicyInference(
        ...
        if missing:
            print(f"[KickingVQController] WARNING – missing weights: {missing}")
        if unexpected:
            print(f"[KickingVQController] WARNING – unexpected keys (ignored): {unexpected}")
        self._policy.eval()

        print(f"[KickingVQController] Loaded checkpoint: {policy_path}")
    ...
    def init_ball(self, model) -> None:
        ...
        print(f"[KickingVQController] Ball: body_id={ball_body_id}, "
