"""
Dribbling VQ Policy controller for MuJoCo sim2sim deployment.

Observation layout (from G1DribblingObservationsCfg.PolicyObsCfg, 383-dim):
  goal_position(3) | ball_rel_pos(3) | ball_vel(3) | ball_to_goal(2)
  | gravity×4(12) | ang_vel×4(12) | jpos×4(116) | jvel×4(116) | actions×4(116)

  goal_position    : [world_dx, world_dy, distance] — world-frame offset + L2 distance
  ball_rel_pos     : ball in robot body frame [x,y,z]
  ball_vel         : world-frame linear velocity [vx,vy,vz]
  ball_to_goal     : (goal_pos - ball_pos) rotated to robot body frame, xy only [2]
...
class _DribblingVQPolicyInference(nn.Module):
    """Deployment-only reconstruction of DownStreamVQPolicy for dribbling task."""
...
class DribblingVQController(BaseController):
    """
    Dribbling VQ policy controller for MuJoCo sim2sim.

    Observation layout (383-dim policy obs):
      goal_position(3) | ball_rel_pos(3) | ball_vel(3) | ball_to_goal(2)
      | gravity×4(12) | ang_vel×4(12) | jpos×4(116) | jvel×4(116) | actions×4(116)

    Call init_ball(model) after creating MujocoSimulator to resolve ball body/joint indices.
    Set goal_pos (world-frame [x,y,z]) before each episode at reset time.
    """

    HISTORY_LEN  = 4
    NUM_JOINTS   = 29
    OBS_DIM      = 383  # 3+3+3+2 + 372
    PROB_OBS_DIM = 372
    ...
    def __init__(
        ...
        self._policy = _DribblingVQPolicyInference(
        ...
        if missing:
            print(f"[DribblingVQController] WARNING – missing weights: {missing}")
        if unexpected:
            print(f"[DribblingVQController] WARNING – unexpected keys (ignored): {unexpected}")
        self._policy.eval()

        print(f"[DribblingVQController] Loaded checkpoint: {policy_path}")
        print(f"  code_dim={code_dim}, num_code={num_code}, lab_lambda={lab_lambda}, use_mp={use_mp}")

        # Goal position (world frame [x,y,z]): set by main script at each episode reset
    ...
    def init_ball(self, model) -> None:
        """Resolve soccer ball joint indices from the compiled MuJoCo model."""
        ...
        print(f"[DribblingVQController] Ball: body_id={ball_body_id}, "
    ...
    def _build_obs(self, mujoco_data) -> tuple[np.ndarray, np.ndarray]:
        ...
        # goal_position: [world_dx, world_dy, distance] — NOT body frame
        rel_goal_w = self.goal_pos[:2] - robot_pos[:2]
        distance   = float(np.linalg.norm(rel_goal_w))
        goal_position = np.array([rel_goal_w[0], rel_goal_w[1], distance], dtype=np.float32)  # [3]

        # ball_relative_position: ball in robot body frame
        rel_ball_w   = ball_pos_w - robot_pos
        ball_rel_pos = _quat_apply_inverse(q_wxyz, rel_ball_w).astype(np.float32)  # [3]

        # ball_velocity: world-frame linear velocity
        ball_vel = ball_vel_w  # [3]

        # ball_to_goal_vector: (goal_pos - ball_pos) in robot body frame, xy only
        btg_w = np.array([
            self.goal_pos[0] - ball_pos_w[0],
            self.goal_pos[1] - ball_pos_w[1],
            0.0,
        ], dtype=np.float32)
        btg_b = _quat_apply_inverse(q_wxyz, btg_w).astype(np.float32)
        ball_to_goal = btg_b[:2]  # [2]
        ...
        obs = np.concatenate([
            goal_position, ball_rel_pos, ball_vel, ball_to_goal,
            hist_gravity, hist_ang_vel, hist_jpos, hist_jvel, hist_actions,
        ])  # 383
