...
# Depth image obs is now handled by the standard ``mjlab.envs.mdp.depth_image``
# (sourced from a :class:`NoisyGroupedRayCasterCamera` sensor whose noise
# pipeline already does distance-Gaussian + dropout + crop+resize + optional
# rect-mask augmentation). The football-local ``depth_image`` /
# ``_apply_random_rect_masks`` were removed during the
# ``depth_pipeline_migration_todo.md`` Phase 0.6 refactor.
