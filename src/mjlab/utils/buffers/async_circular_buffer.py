"""Per-batch asynchronous circular buffer.

Ported from ``mjlab-prior-main/src/mjlab/utils/buffers/async_circular_buffer.py``
verbatim — the only mjlab-prior change is the parent ``CircularBuffer``
import path (same here).

Compared to the synchronous :class:`CircularBuffer` (one global write
pointer for all batches), :class:`AsyncCircularBuffer` keeps a **per-batch
write pointer**. That lets a sensor with ``update_period > 0`` refresh
the buffer on a subset of envs each tick (the ones whose virtual frame
has arrived) without disturbing the chronological order for the others.
"""
