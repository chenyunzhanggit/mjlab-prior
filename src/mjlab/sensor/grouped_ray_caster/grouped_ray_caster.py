...
from mjlab.sensor.raycast_sensor import RayCastData, RayCastSensor
...
class GroupedRayCaster(RayCastSensor):
  ...
  def initialize(self, mj_model, model, data, device: str) -> None:
    ...
    self._attachment_frame_metadata = self._resolve_attachment_frame_metadata(
      mj_model, device
    )
    ...
    if self._mesh_filter_epsilon <= 0.0:
      raise ValueError(
        f"mesh_filter_epsilon must be > 0.0, got {self.cfg.mesh_filter_epsilon}."
      )

    self._mesh_filter_max_hops = int(self.cfg.mesh_filter_max_hops)
    if self._mesh_filter_max_hops < 1:
      raise ValueError(
        f"mesh_filter_max_hops must be >= 1, got {self.cfg.mesh_filter_max_hops}."
      )
    ...
    self.ray_starts = (
      self._local_offsets.unsqueeze(0).repeat(self._num_envs, 1, 1).clone()
    )
    self.ray_directions = (
      self._local_directions.unsqueeze(0).repeat(self._num_envs, 1, 1).clone()
    )
  ...
  def _resolve_attachment_frame_metadata(
    ...
    if frame_type == "body":
      ...
      frame_local_pos = torch.tensor(
        mj_model.site_pos[obj_id], device=device, dtype=torch.float32
      )
    elif frame_type == "geom":
      obj_id = mj_model.geom(frame_name).id
      body_id = int(mj_model.geom_bodyid[obj_id])
      frame_local_pos = torch.tensor(
        mj_model.geom_pos[obj_id], device=device, dtype=torch.float32
      )
    else:
      raise ValueError(
  ...
  def _initialize_mesh_path_filter(self, mj_model, device: str) -> None:
    ...
    allowed_geom_lut[
      torch.tensor(sorted(set(allowed_geom_ids)), device=device, dtype=torch.long)
    ] = True
  ...
  def _apply_hit_filter_and_continue(self) -> None:
    ...
    current_origins[reject_mask] = (
      hit_pos_w[reject_mask] + world_rays[reject_mask] * eps
    )
    ...
    if not torch.any(active):
      self._update_mutable_raycast_outputs(
        final_distances, final_hit_pos, final_normals
      )
      return
    ...
    for _ in range(max_hops):
      ...
      (
        ...
        active_geom_ids,
        active_normals,
      ) = self._raycast_active_rays(
        active=active, current_origins=current_origins, world_rays=world_rays
      )
      ...
      active_total_distances = (
        traveled[active_env_ids, active_ray_ids] + active_distances
      )

      active_accept = (
        active_hit & active_allowed & (active_total_distances > self._min_distance)
      )
      if torch.any(active_accept):
        accept_env_ids = active_env_ids[active_accept]
        accept_ray_ids = active_ray_ids[active_accept]
        final_distances[accept_env_ids, accept_ray_ids] = active_total_distances[
          active_accept
        ]
      ...
      current_origins[continue_env_ids, continue_ray_ids] = (
        continue_hit_pos + continue_dirs * eps
      )
      ...
      if torch.any(continue_valid):
        active[continue_env_ids[continue_valid], continue_ray_ids[continue_valid]] = (
          True
        )
  ...
  def _ensure_active_hop_buffers(self, capacity: int) -> None:
    ...
    self._active_hop_pnt = torch.empty(
      (self._num_envs, capacity, 3), device=device, dtype=torch.float32
    )
    self._active_hop_vec = torch.empty(
      (self._num_envs, capacity, 3), device=device, dtype=torch.float32
    )
    self._active_hop_dist = torch.empty(
      (self._num_envs, capacity), device=device, dtype=torch.float32
    )
    self._active_hop_geomid = torch.empty(
      (self._num_envs, capacity), device=device, dtype=torch.int32
    )
    self._active_hop_normal = torch.empty(
      (self._num_envs, capacity, 3), device=device, dtype=torch.float32
    )
  ...
  def _raycast_active_rays(
    ...
    *,
    ...
    active_hop_pnt[active_env_ids, active_slot_ids] = current_origins[
      active_env_ids, active_ray_ids
    ]
    active_hop_vec[active_env_ids, active_slot_ids] = world_rays[
      active_env_ids, active_ray_ids
    ]
  ...
  def _write_world_rays_to_backend(
    self, world_origins: torch.Tensor, world_rays: torch.Tensor
  ) -> None:
  ...
  def _get_mutable_raycast_outputs(
    self,
  ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    assert (
      self._distances is not None
      and self._hit_pos_w is not None
      and self._normals_w is not None
    )
  ...
  def _update_mutable_raycast_outputs(
    ...
    backend_distances, backend_hit_pos_w, backend_normals_w = (
      self._get_mutable_raycast_outputs()
    )
