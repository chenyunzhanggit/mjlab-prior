...
class GroupedRayCasterCamera(GroupedRayCaster):
  ...
  def initialize(self, mj_model, model, data, device: str) -> None:
    ...
    self._elapsed_since_refresh = torch.zeros(
      self._num_envs, device=device, dtype=torch.float32
    )
  ...
  def set_world_poses(
    ...
    if positions is not None:
      ...
      self._offset_pos[env_ids] = math_utils.quat_apply(
        math_utils.quat_inv(quat_w), pos_offset_world_frame
      )
    if orientations is not None:
      ...
      self._offset_quat[env_ids] = math_utils.quat_mul(
        math_utils.quat_inv(quat_w), quat_w_set
      )
  ...
  def set_world_poses_from_view(
    ...
    orientations = math_utils.quat_from_matrix(
      math_utils.create_rotation_matrix_from_view(
        eyes, targets, up_axis="Z", device=device
      )
    )
  ...
  def prepare_rays(self) -> None:
    ...
    ray_starts_w = math_utils.quat_apply(quat_w_expanded, ray_starts_flat).view(
  ...
  def postprocess_rays(self) -> None:
    ...
    if self._update_period_s > 0.0:
      ...
      if torch.any(stale_mask):
        ...
        for data_type in self.cfg.data_types:
          stale_cached_outputs[data_type] = self._camera_data.output[data_type][
            stale_env_ids
          ].clone()
    ...
    if "distance_to_image_plane" in self.cfg.data_types:
      ...
      if self.cfg.depth_clipping_behavior == "max":
        distance_to_image_plane = torch.clip(
          distance_to_image_plane, max=self.cfg.max_distance
        )
        distance_to_image_plane[torch.isnan(distance_to_image_plane)] = (
      ...
      self._camera_data.output["distance_to_image_plane"] = (
        distance_to_image_plane.view(-1, *self.image_shape, 1)
      )
    ...
    if "distance_to_camera" in self.cfg.data_types:
      ...
      self._camera_data.output["distance_to_camera"] = ray_depth_camera.view(
        -1, *self.image_shape, 1
      )

    if "normals" in self.cfg.data_types:
      self._camera_data.output["normals"] = raycast_data.normals_w.view(
  ...
  def _create_buffers(self):
    ...
    # -- intrinsic matrix
    self._camera_data.intrinsic_matrices = torch.zeros(
      (self._num_envs, 3, 3), device=device
    )
    ...
    for name in self.cfg.data_types:
      if name in ["distance_to_image_plane", "distance_to_camera"]:
        ...
        raise ValueError(
          f"Received unknown data type: {name}. Please check the configuration."
        )
      # allocate tensor to store the data
      self._camera_data.output[name] = torch.zeros(
        (self._num_envs, *shape), device=device
      )
  ...
  def _compute_intrinsic_matrices(self):
    ...
    if self.cfg.focal_length is not None and self.cfg.horizontal_aperture is not None:
      ...
      if vertical_aperture is None:
        vertical_aperture = (
          self.cfg.horizontal_aperture * pattern_cfg.height / pattern_cfg.width
        )
  ...
  def _compute_pinhole_rays_from_intrinsics(
    ...
    grid = torch.meshgrid(
      torch.arange(
        start=0, end=self.cfg.pattern.width, dtype=torch.int32, device=device
      ),
      torch.arange(
        start=0, end=self.cfg.pattern.height, dtype=torch.int32, device=device
      ),
      indexing="xy",
    )
    ...
    transform_vec = (
      torch.tensor([1, -1, -1], device=device, dtype=intrinsic_matrices.dtype)
      .unsqueeze(0)
      .unsqueeze(2)
    )
  ...
  def _compute_camera_world_poses(
    self, env_ids: Sequence[int] | torch.Tensor
  ) -> tuple[torch.Tensor, torch.Tensor]:
  ...
  def _sense_on_demand(self) -> None:
    ...
    if (
      ...
      or self._runtime_wp_device is None
    ):
      return
    self.prepare_rays()
