#!/usr/bin/env bash
# Play the trained VQ-VAE motion-prior student under Path 3 deploy:
#   prop_obs -> motion_prior -> z(code_dim) -> decoder([prop, z]) -> action
#
...
#   Mjlab-MotionPrior-VQ-Flat-Unitree-G1   (default) — flat terrain
#   Mjlab-MotionPrior-VQ-Rough-Unitree-G1            — rough terrain
#
# Either way the deploy chain ignores teacher / command obs. To watch the
# VQ deploy chain on rough terrain:
#   TASK=Mjlab-MotionPrior-VQ-Rough-Unitree-G1 ./play_motion_prior_vq_deploy.sh
set -euo pipefail

# ---- override via env vars or edit defaults below ----
RUN="${RUN:-/home/lenovo/project/mjlab_prior/logs/rsl_rl/g1_motion_prior_vq/2026-05-06_15-39-54}"
CKPT="${CKPT:-model_9000.pt}"
